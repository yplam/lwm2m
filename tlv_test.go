...
func TestSingleObjectTLV(t *testing.T) {
	...
	i8, err := v[7].Int8Val()
	assert.Nil(t, err)
	assert.Equal(t, i8, int8(0x64))
	u8, err := v[7].Uint8Val()
	assert.Nil(t, err)
	assert.Equal(t, u8, uint8(0x64))

	i16, err := v[5].Children[0].Int16Val()
	assert.Nil(t, err)
	assert.Equal(t, i16, int16(0x0ed8))
	u16, err := v[5].Children[1].Uint16Val()
	assert.Nil(t, err)
	assert.Equal(t, u16, uint16(0x1388))

	u32, err := v[10].Uint32Val()
	assert.Nil(t, err)
	assert.Equal(t, u32, uint32(0x5182428F))
	i32, err := v[10].Int32Val()
	assert.Nil(t, err)
	assert.Equal(t, i32, int32(0x5182428F))
